					...
					>
						<div className="flex items-center justify-between">
							...
							<div className="flex items-center ml-4">
								<span className="text-sm text-gray-500 mr-4">{url.clicks} clicks</span>
								<button onClick={() => handleCopy(url.shortUrl, url.id)} className="p-2 hover:bg-gray-100 rounded-full transition-colors">
									{copiedId === url.id ? (
										<motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-green-600 text-sm">
											Copied!
										</motion.span>
									) : (
